...
DYNAMODB_TABLE = 'cloud-governance-incident-log'
...
def api_cpu():
    try:
        ...
        return jsonify({
            ...
            'status': 'alarm' if current > 70 else 'warning' if current > 50 else 'ok'
        })
...
def api_lambda_stats():
    try:
        ...
        return jsonify({
            'self_healing': {
                'invocations': get_lambda_metric('cloud-governance-self-healing', 'Invocations'),
                'errors': get_lambda_metric('cloud-governance-self-healing', 'Errors')
            },
            'finops': {
                'invocations': get_lambda_metric('cloud-governance-finops', 'Invocations'),
